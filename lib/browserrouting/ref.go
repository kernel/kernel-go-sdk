...
func (r Ref) Normalize() (Ref, error) {
	if strings.TrimSpace(r.BaseURL) == "" {
		return Ref{}, fmt.Errorf("browserrouting: base_url is required")
	}
	if strings.TrimSpace(r.SessionID) == "" {
		return Ref{}, fmt.Errorf("browserrouting: session_id is required")
	}
	out := r
	...
	if out.JWT == "" {
		src := out.CdpWsURL
		if src == "" {
			return Ref{}, fmt.Errorf("browserrouting: jwt or cdp_ws_url is required")
		}
